$(function () {
    ...
    $(function () {
        ...
        var o = $(".sidebar-wrapper .tab-content a");

        o.each(function () {
            const u = new URL(this.href);

            // Ambil path utama dari URL halaman saat ini
            const currentBase = `${e.origin}/${
                e.pathname.split("/").filter(Boolean)[0] || ""
            }`;
            // Ambil path utama dari setiap link di sidebar
            const linkBase = `${u.origin}/${
                u.pathname.split("/").filter(Boolean)[0] || ""
            }`;

            // Cek apakah link cocok dengan URL penuh atau base path
            if (this.href === e.href || linkBase === currentBase) {
